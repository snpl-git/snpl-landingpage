...
import { EmailAuthRequestSchema, emailOtpCredentials } from '@/lib/auth-input'

export async function POST(request: Request) {
  const limit = await consumeRateLimit(RATE_LIMITS.authEmailSend, trustedClientIp(request.headers))
  ...
  try {
    const parsed = EmailAuthRequestSchema.safeParse(await readJsonBody(request, 4096))
    if (!parsed.success) return NextResponse.json({ error: 'Complete the security check and enter a valid email.' }, { status: 400 })
    const supabase = await createAuthClient()
    const { error } = await supabase.auth.signInWithOtp(emailOtpCredentials(parsed.data))
