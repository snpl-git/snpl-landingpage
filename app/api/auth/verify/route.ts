...
export async function POST(request: Request) {
  const limit = await consumeRateLimit(RATE_LIMITS.authOtpVerify, trustedClientIp(request.headers))
  ...
  try {
    ...
    const { data, error } = await supabase.auth.verifyOtp({ ...parsed.data, type: 'sms' })
